...
Database migrations run automatically when the new server starts, and the instance is unavailable while they run. Migrations are forward-only: if the newer release added one, an older image refuses to start against the migrated database and exits. To roll back, stop the stack, [restore the pre-update backup](../manage-data-and-backups/#restore-standalone-docker-compose), set the `image:` tag back to the previous release, and start the stack again.
