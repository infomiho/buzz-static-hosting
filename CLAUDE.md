...
For wildcard certificates, add a separate `cloudflare` DNS-01 resolver with its own ACME storage to Coolify's Traefik proxy and a single `wildcard-certs` router for `BUZZ_DOMAIN` and `*.BUZZ_DOMAIN`; Coolify's default `letsencrypt` resolver stays untouched, so other applications on the proxy are unaffected. Buzz app labels should set `tls=true` without `tls.certresolver`; otherwise Traefik creates duplicate ACME challenges for the same `_acme-challenge` record. The original personal instance predates this and switched `letsencrypt` itself to DNS-01; both shapes work, do not migrate it.
